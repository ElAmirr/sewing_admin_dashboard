...
// repositories/MetadataRepository.js
var import_promises = __toESM(require("fs/promises"), 1);
var import_path = __toESM(require("path"), 1);
var DATA_DIR = process.env.DATA_PATH || import_path.default.resolve("data");
...
// repositories/LogRepository.js
var DATA_DIR2 = process.env.DATA_PATH || import_path2.default.resolve("data");
console.log(`[LogRepository] Using DATA_DIR: ${DATA_DIR2}`);
var LogRepository = class {
  ...
  async getLogs(startDate, endDate) {
    ...
    const promises = machines.map(async (machine) => {
      const machineDir = import_path2.default.join(DATA_DIR2, `machine_${machine.machine_id}`);
      const filePromises = filesToRead.map(async (filename) => {
        const filePath = import_path2.default.join(machineDir, filename);
        try {
  ...
  async createLog(logData) {
    ...
    const machineDir = import_path2.default.join(DATA_DIR2, `machine_${machine_id}`);
    const filePath = import_path2.default.join(machineDir, `${dateStr}.json`);
    await import_promises2.default.mkdir(machineDir, { recursive: true });
    let logs = [];
    try {
      const content = await import_promises2.default.readFile(filePath, "utf-8");
    ...
    logs.push(newLog);
    await import_promises2.default.writeFile(filePath, JSON.stringify(logs, null, 2));
    return nextLogId;
  }
};
  ...
  try {
    ...
    const transformed = rawLogs.map((log) => {
      ...
      return {
        ...
        operator: operator ? { name: operator.name, badge: operator.badge } : null,
      ...
      (a, b) => new Date(b.cycle_start_time) - new Date(a.cycle_start_time) || new Date(b.operator_press_time) - new Date(a.operator_press_time)
    );
...
var getSessions = async (req, res) => {
  try {
    ...
    const DATA_DIR3 = process.env.DATA_PATH || import_path3.default.resolve("data");
    const SESSION_FILE = import_path3.default.join(DATA_DIR3, "machine_sessions.json");
    let sessions = [];
    try {
      const content = await import_promises3.default.readFile(SESSION_FILE, "utf-8");
...
var logs_routes_default = router;
...
app.use("/api/logs", logs_routes_default);
...
var getOperators = async (req, res) => {
  try {
...
var getSupervisors = async (req, res) => {
  try {
...
var addSupervisor = async (req, res) => {
  try {
...
var deleteSupervisor = async (req, res) => {
  try {
    const { id } = req.params;
    const supervisors = await metadataRepository.getSupervisors();
...
var deleteMachine = async (req, res) => {
  try {
    const { id } = req.params;
    const machines = await metadataRepository.getMachines();
...
var updateSupervisor = async (req, res) => {
  try {
    const { id } = req.params;
...
var updateMachine = async (req, res) => {
  try {
    const { id } = req.params;
    const { code } = req.body;
    const machines = await metadataRepository.getMachines();
...
// server.js
var import_cors2 = __toESM(require_lib2(), 1);
var import_body_parser = __toESM(require("body-parser"), 1);
var PORT = 3001;
app_default.use((0, import_cors2.default)());
app_default.use(import_body_parser.default.json());
app_default.get("/api/logs", getLogs);
app_default.post("/api/logs", createLog);
app_default.get("/api/sessions", getSessions);
app_default.get("/api/metadata/operators", getOperators);
app_default.post("/api/metadata/operators", addOperator);
app_default.put("/api/metadata/operators/:id", updateOperator);
app_default.delete("/api/metadata/operators/:id", deleteOperator);
app_default.get("/api/metadata/supervisors", getSupervisors);
app_default.post("/api/metadata/supervisors", addSupervisor);
app_default.put("/api/metadata/supervisors/:id", updateSupervisor);
app_default.delete("/api/metadata/supervisors/:id", deleteSupervisor);
app_default.get("/api/metadata/machines", getMachines);
app_default.post("/api/metadata/machines", addMachine);
app_default.put("/api/metadata/machines/:id", updateMachine);
app_default.delete("/api/metadata/machines/:id", deleteMachine);
